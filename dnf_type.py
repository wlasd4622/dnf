...
def dqandpath(dnf, qq_id):
    ...
    pic_path = curPath + '\\Picture\\'
    dnf.SetDict(0, curPath +'\\配置\\dnf.txt')
    dnf_path = dnf.ReadIni("DNF路径","路径",curPath + '\\配置\\配置.ini')
    qq_path = dnf.ReadIni("账号密码",qq_id, curPath + '\\配置\\配置.ini')
